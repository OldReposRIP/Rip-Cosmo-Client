...
public class LongArgumentType implements ArgumentType {
    ...
    private static final Collection EXAMPLES = Arrays.asList(new String[] { "0", "123", "-123"});
    ...
    public static long getLong(CommandContext context, String name) {
        return ((Long) context.getArgument(name, Long.TYPE)).longValue();
    }
    ...
    public Long parse(StringReader reader) throws CommandSyntaxException {
        ...
        if (result < this.minimum) {
            ...
            throw CommandSyntaxException.BUILT_IN_EXCEPTIONS.longTooLow().createWithContext(reader, Long.valueOf(result), Long.valueOf(this.minimum));
        } else if (result > this.maximum) {
            reader.setCursor(start);
            throw CommandSyntaxException.BUILT_IN_EXCEPTIONS.longTooHigh().createWithContext(reader, Long.valueOf(result), Long.valueOf(this.maximum));
        } else {
