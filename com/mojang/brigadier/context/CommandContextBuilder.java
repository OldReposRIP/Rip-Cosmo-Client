        ...
        if (this.range.getStart() <= cursor) {
            if (this.range.getEnd() < cursor) {
                if (this.child != null) {
                    ...
                    ParsedCommandNode prev1 = (ParsedCommandNode) this.nodes.get(this.nodes.size() - 1);

                    return new SuggestionContext(prev1.getNode(), prev1.getRange().getEnd() + 1);
                } else {
                ...
                for (Iterator iterator = this.nodes.iterator(); iterator.hasNext(); prev = node.getNode()) {
                    node = (ParsedCommandNode) iterator.next();
