    ...
    void startTableArray(Identifier identifier, AtomicInteger line) {
        ...
        for (int i = 0; i < tableParts.length; ++i) {
            ...
            if (currentContainer.get(tablePart) instanceof Container.TableArray) {
                Container.TableArray newContainer = (Container.TableArray) currentContainer.get(tablePart);

                this.stack.push(newContainer);
                if (i == tableParts.length - 1) {
                    newContainer.put(tablePart, new Container.Table());
                }
                ...
                Container container = (Container) currentContainer.get(tablePart);
                ...
                Object object = i == tableParts.length - 1 ? new Container.TableArray() : new Container.Table();

                this.addValue(tablePart, object, line);
                this.stack.push(object);
                if (object instanceof Container.TableArray) {
                    this.stack.push(((Container.TableArray) object).getCurrent());
                }
            }
        }
